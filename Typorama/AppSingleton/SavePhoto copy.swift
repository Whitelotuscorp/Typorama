...
//                layer.transform = .identity
                ...
                if let img_LyMain : UIImage = infoL.image, img_LyMain.size.width > 0 && img_LyMain.size.height > 0 {
                                        
                    let gap = layer.contentView.frame.origin.x
                    
                    var w_New = layer.frame.size.width - (2 * gap)
                    var h_New = layer.frame.size.height - (2 * gap)
                    
                    var w_Diff : CGFloat = 0
                    var h_Diff : CGFloat = 0
                     
                    let sizeLYImg = img_LyMain.size
                    if CGFloat(img_LyMain.size.width) > CGFloat(img_LyMain.size.height) {
                         
                        w_New = h_New * sizeLYImg.width / sizeLYImg.height
                        w_Diff = (layer.contentView.frame.width - w_New) / 2
                    }
                    else {
                         
                        h_New = w_New * sizeLYImg.height / sizeLYImg.width
                        h_Diff = (layer.contentView.frame.height - h_New) / 2
                    }
    
//                    let x_LY: CGFloat = (x_New * size!.width) / imageview!.frame.size.width
//                    let y_LY: CGFloat = (y_New * size!.height) / imageview!.frame.size.height
//                    let w_LY: CGFloat = (w_New * size!.width) / imageview!.frame.size.width
//                    let h_LY: CGFloat = (h_New * size!.height) / imageview!.frame.size.height
//
//                    let area_LY = CGRect(x: x_LY, y: y_LY, width: w_LY, height: h_LY)
                    layer.transform = transform
                    
                    let x_LY: CGFloat = ((layer.frame.origin.x + gap + w_Diff) * size!.width) / view!.frame.size.width
                    let y_LY: CGFloat = ((layer.frame.origin.y + gap + h_Diff) * size!.height) / view!.frame.size.height
                    let w_LY: CGFloat = (w_New * size!.width) / view!.frame.size.width
                    let h_LY: CGFloat = (h_New * size!.height) / view!.frame.size.height
