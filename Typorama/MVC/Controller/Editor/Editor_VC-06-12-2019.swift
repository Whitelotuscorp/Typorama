...
//        infoL.fontFamily = ARRAY_Font[AppSingletonObj.randomNumber(min: 0, max: ARRAY_Font.count - 1)]
    ...
    func updateLayer1(layer: ZDStickerView, infoS: infoStyle, isAnimate: Bool = true, width: CGFloat = 0) {
        ...
        for info in effect.texts {
            ...
            var attrs = [NSAttributedString.Key.font : UIFont(name: fontFamily, size: largestFontSize) as Any,
                         NSAttributedString.Key.foregroundColor : textColor,
                         NSAttributedString.Key.paragraphStyle : paragraphStyle,
                         NSAttributedString.Key.kern : NSNumber(value: Float(infoS.charcterSpacing))]
            ...
            if info.isLine == true {
                ...
                while(isIncFont == true){
                    ...
                    attrs[NSAttributedString.Key.font] = UIFont(name: fontFamily, size: largestFontSize) as Any
                    sz_Add = (str_Text as NSString).size(withAttributes: attrs)
            ...
            if info.isLine == true || isClearLabel == true {
                ...
                newRect = CGRect(x: x, y: y + ln_pd, width: w_Label, height: sz_Add.height + h_LabelU)
            }
            else {
                
                newRect = CGRect(x: x, y: y + pd + ln_PrevLine, width: w_Label, height: sz_Add.height + h_LabelU + (pd * 2))
            }
            ...
            if isClearLabel == true && info.isLine != true {
                ...
                objCTLbl.text = str_Text.uppercased()
                ...
                if info.isLine == true {
                    ...
                    img = renderer.image { ctx in
                        ...
                        str_Text.draw(with: CGRect(x: 0, y: pd, width: sz_Add.width, height: sz_Add.height + pd * 2), options: .usesLineFragmentOrigin, attributes: attrs, context: nil)
                    }
                }
